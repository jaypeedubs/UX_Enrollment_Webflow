...
- The `.ppt` template the user mentioned never arrived in uploads — slide layouts in `slides/index.html` are derived from the website's typography and color, not from the actual deck file. If the user provides the template later, re-derive from it.
- Two course-director names in the marketing UI kit's faculty grid are placeholders (`Adrien Eshraghi, MD`, `Cedric Navarro, AuD`, `Philomin Joseph, AuD`). The Eshraghi name is real (a UMiami CI surgeon), the others are inferred — verify with the user before publishing.
- The faculty photographs `img-cedric.webp`, `img-philomin.webp`, `img-shvets.webp` come from the Webflow CMS and may not correspond to the named individuals. Check before using in production.

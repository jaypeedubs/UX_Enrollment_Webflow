...
prompt_action() {
  ...
  while true; do
    ...
    echo "  [a] abort"
    echo
    echo "Apply to all remaining files:"
    echo "  [B] backup and overwrite ALL remaining"
    echo "  [S] skip ALL remaining"
    echo "  [O] overwrite ALL remaining without backup"
    read -r -p "> " answer
    ...
    case "$answer" in
      ...
      a|abort)
        ...
        ;;
      B)
        GLOBAL_ACTION="backup"
        echo "Global action set: backup all remaining"
        echo "backup"
        return 0
        ;;
      S)
        GLOBAL_ACTION="skip"
        echo "Global action set: skip all remaining"
        echo "skip"
        return 0
        ;;
      O)
        GLOBAL_ACTION="overwrite"
        echo "Global action set: overwrite all remaining without backup"
        echo "overwrite"
        return 0
        ;;
      *)
        echo "Please enter b, s, o, a, B, S, or O."
        ;;
    esac
  done
}
...
write_skill() {
  local target="$1"
  local tmp action backup_target

  mkdir -p "$(dirname "$target")"

  if [[ -f "$target" ]]; then
    action="$(prompt_action "$target")"

    case "$action" in
      backup)
        backup_target="$BACKUP_DIR/${target#"$ROOT"/}"
      ...
      skip)
        ...
        ;;
      overwrite)
        echo "Overwriting without backup: $target"
        updated=$((updated + 1))
        ;;
      abort)
